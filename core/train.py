...
def train_cs_my(net, criterion, optimizer, trainloader, trainout_loader, epoch=None, **options):
    net.train()
    losses = AverageMeter()
    ...
    loss_all = 0
    for batch_idx, ((data, labels), (data_out, labels_out)) in enumerate(zip(trainloader, trainout_loader)):
        if options['use_gpu']:
            data, labels = data.cuda(), labels.cuda()
            data_out, labels_out = data_out.cuda(), labels_out.cuda()
...
def train_cs_my_abn(net, criterion, optimizer, trainloader, trainout_loader, epoch=None, **options):
    net.train()
    losses = AverageMeter()
    ...
    for batch_idx, ((data, labels), (data_out, labels_out)) in enumerate(zip(trainloader, trainout_loader)):
        ...
        with torch.set_grad_enabled(True):
            optimizer.zero_grad()
            x, y = net(data, True, in_domain=True)
            _, loss = criterion(x, y, labels)

            x, y = net(data_out, True, in_domain=False)
            # x, y = net(fake, True, 1 * torch.ones(data.shape[0], dtype=torch.long).cuda())
            F_loss_fake = criterion.fake_loss(x).mean()
            total_loss = loss + options['beta'] * F_loss_fake
...
def train_cs_gan_my(net, netD, netG, criterion, criterionD, optimizer, optimizerD, optimizerG, 
        trainloader, epoch=None, **options):
    print('train with confusing samples')
    losses, lossesG, lossesD = AverageMeter(), AverageMeter(), AverageMeter()
    ...
    for batch_idx, (data, labels) in enumerate(trainloader):
        ...
        noise = torch.FloatTensor(data.size(0), options['nz']).normal_(0, 1).cuda()
        if options['use_gpu']:
            noise = noise.cuda()
        noise = Variable(noise)
        fake = netG(noise)
        ...
        x, y = net(fake, True)
        F_loss_fake = criterion.fake_loss(x).mean()
        total_loss = loss + options['beta_gan'] * F_loss_fake
        total_loss.backward()
        optimizer.step()
